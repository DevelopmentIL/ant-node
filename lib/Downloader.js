...
proto._peerStable = function(peer) {
	if(!peer.stable) {
		peer.stable = true;
		this._unstablePeers--;
	}
...
proto._peerUnstable = function(peer) {
	if(peer.stable) {
		peer.stable = false;
		this._unstablePeers++;
	}
...
proto._requestSuccess = function(request, peer) {
	peer.errors = 0;
	this._peerStable(peer);
	
	if(this._unstablePeers === 0) {
		this._peerUnstable(peer);
